...
(define-constant ERR_STREAM_PAUSED (err u105))
...
(define-public (withdraw (id uint))
  (let ((stream (unwrap! (map-get? streams id) ERR_NO_STREAM)))
    (begin
      ...
      (let (
            (available (unwrap-panic (get-available-balance id)))
          )
        (if (<= available u0)
            (ok u0)
            (begin
              (try! (stx-transfer? available (as-contract tx-sender) tx-sender))
              (map-set streams id (merge stream {withdrawn: (+ (get withdrawn stream) available)}))
              (ok available)
            )
        )
      )
...
;; === Pause stream (only employer) ===
(define-public (pause-stream (id uint))
  (let ((stream (unwrap! (map-get? streams id) ERR_NO_STREAM)))
    (begin
      (asserts! (is-eq (get employer stream) tx-sender) ERR_NOT_EMPLOYER)
      (map-set streams id (merge stream {paused: true}))
      (ok true)
    )
  )
)

;; === Resume stream (only employer) ===
(define-public (resume-stream (id uint))
  (let ((stream (unwrap! (map-get? streams id) ERR_NO_STREAM)))
    (begin
      (asserts! (is-eq (get employer stream) tx-sender) ERR_NOT_EMPLOYER)
      (map-set streams id (merge stream {paused: false}))
      (ok true)
    )
  )
)

;; === Update recipient (only employer) ===
(define-public (update-recipient (id uint) (new-recipient principal))
  (let ((stream (unwrap! (map-get? streams id) ERR_NO_STREAM)))
    (begin
      (asserts! (is-eq (get employer stream) tx-sender) ERR_NOT_EMPLOYER)
      (map-set streams id (merge stream {recipient: new-recipient}))
      (ok true)
    )
  )
)

;; === Extend stream with more funds and duration ===
(define-public (extend-stream (id uint) (extra-duration uint) (extra-amount uint))
  (let ((stream (unwrap! (map-get? streams id) ERR_NO_STREAM)))
    (begin
      (asserts! (is-eq (get employer stream) tx-sender) ERR_NOT_EMPLOYER)
      (try! (stx-transfer? extra-amount tx-sender (as-contract tx-sender)))
      (map-set streams id (merge stream {
        total-amount: (+ (get total-amount stream) extra-amount),
        end-block: (+ (get end-block stream) extra-duration)
      }))
      (ok true)
    )
  )
...
;; === Employer withdraw on behalf (automation) ===
(define-public (force-withdraw (id uint))
  (let ((stream (unwrap! (map-get? streams id) ERR_NO_STREAM)))
    (begin
      (asserts! (is-eq (get employer stream) tx-sender) ERR_NOT_EMPLOYER)
      (let ((available (unwrap-panic (get-available-balance id))))
        (if (<= available u0)
            (ok u0)
            (begin
...
;; === View available balance (read-only) ===
(define-read-only (get-available-balance (id uint))
  (let ((stream (unwrap! (map-get? streams id) ERR_NO_STREAM)))
    (let (
